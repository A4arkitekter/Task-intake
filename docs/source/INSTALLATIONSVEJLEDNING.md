...
3. Setup viser den mappe, optagelser hentes fra, typisk:

```text
C:\Users\<windows-login>\Dropbox\Apps\ASRRecordings
```

   Tryk Enter, hvis stien er rigtig. Ellers skriv den rigtige mappe. Det er den mappe, ASR Voice Recorder synker til via Dropbox (Android eller iPhone).
